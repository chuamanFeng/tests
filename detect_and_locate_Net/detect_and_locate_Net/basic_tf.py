...
def conv2d_preload(inputs,                   
  ...
  with tf.variable_scope(scope) as sc:
      ...
      stride_h, stride_w = stride
      kernel=get_conv_kernel(data_dict,sc)
      outputs = tf.nn.conv2d(inputs, kernel,
                             [1, stride_h, stride_w, 1],
                             padding=padding)
     
      biases = get_bias(data_dict,sc)  
      outputs = tf.nn.bias_add(outputs, biases)
      ...
      return outputs
#***********************************************************************************#
def fully_connected_preload(inputs,
                        num_outputs,
                        scope,
                        data_dict,
                        use_xavier=True,
  ...
  with tf.variable_scope(scope) as sc:
    ...
    weights = get_fc_weight(data_dict,sc)  
    biases = get_bias(data_dict,sc)  
...
def get_conv_kernel(data_dict, name): 

    return tf.constant(data_dict[name][0], name="kernel")  
#-------------------------------------------------------------pooling----------------------------------------------------------#
#***************************max-pooling2D***************************#
