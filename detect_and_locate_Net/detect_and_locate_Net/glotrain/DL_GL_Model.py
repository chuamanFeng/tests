...
def get_loss_G(pred,labels,scales,img_size,scope):
    ...
    with tf.variable_scope(scope):
        ...
        coord_loss =tf.reduce_mean((((true_locate[:,0]-pred_locate[:,0])/img_size[0])**2+((true_locate[:,1]-pred_locate[:,1])**2)/img_size[1]**2),-1)* scales[0]
        scale_loss =tf.reduce_mean((((true_locate[:,2]-pred_locate[:,2])/img_size[0])**2)+((true_locate[:,3]-pred_locate[:,3])/img_size[1])**2)*scales[1]   
