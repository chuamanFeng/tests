...
parser.add_argument('--model', default='DL_GL_Model', help='Model name')
parser.add_argument('--log_dir', default='H:\DLtrue', help='Log dir [default: log]')
parser.add_argument('--stan_size', default=[224,224], help='lenth&width for croping&resizing')
parser.add_argument('--scales', default=[3,3,4], help='for loss_calc,[class,object,ant_object,coord]')
...
'''
LOG_FOUT = open(os.path.join(LOG_DIR, 'log_GL_train.txt'), 'w')
...
#save_dir='H:\DLtest\B'
data_dir='H:\DLtrue\packtr'   #HERE WE GONNA USE TFRecords
data_dir_test='H:\DLtrue\packte'
data_dir_vali='H:\DLtrue\packva'
   ...
   C part
    ...
    with tf.Graph().as_default():
        ...
        with tf.variable_scope('input_placeholder'):
            img_pl, label_pl= MODEL.placeholder_inputs(BATCH_SIZE,STAN_SIZE,CHANNEL)
        ...
        with tf.variable_scope('scalars'):
            ...
            pred= MODEL.basic_detectModel(img_pl,is_training_pl,bn_decay,CLASS_NUM)
            loss = MODEL.get_loss_G(pred,label_pl,SCALES,STAN_SIZE,'loss_calc')       
        ...
        with tf.variable_scope('data_packs_ops'):
            ...
            for i ,path in enumerate(FULL_FILE_LIST[0]):
                list_img,list_label=tl.TF_loader_multi_glo(path,[STAN_SIZE[0],STAN_SIZE[1],CHANNEL],5,num_epochs=None)
    ...
    for i in range(file_lenth):  #tfrecords   
        ...
        with tf.variable_scope('test/vali_layer') as sc:    
            while idx+batch_size<num:              
                ...
                #then cook  
                pred_val = np.argmax(pred_val[:,4:], 1)    
                correct = np.sum(pred_val ==labels[:,4])  #in this line ,labels have been changed to float32
                total_correct += correct
                total_seen += BATCH_SIZE
                loss_sum += (loss_val*BATCH_SIZE)
                #extra loop:
                #count how many times each class has occured in prediction,and numbering the correct ones
                templ=np.squeeze(labels[:,4])   #(B)               
