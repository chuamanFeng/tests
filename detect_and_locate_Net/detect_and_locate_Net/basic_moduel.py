...
def placeholder_inputs_pre(batch_size,img_size,channel):
    ...
    img_pl=tf.placeholder(shape=[batch_size,img_size[0],img_size[1],channel],dtype=tf.int32)
    label_pl=tf.placeholder(shape=[batch_size],dtype=tf.float32)
...
def loss_calc_loop(object_idx, object_num, loss, predict, labels, image_size,cell_size,num_class,box_num,scales):  
    ...
    with tf.variable_scope('calc_iou_predict_truth_confidence'):
