...
#-------------------------------------------------------------global pack
def get_all_done(main_path,TF_savepath,img_size,file_size,brt_hue_round=1,crop_round=2,resize=True,raw_size=[512,512],new_size=[224,224]):
    ...
    for j,folder in enumerate(file_folders_class):
        ...
        img,label,img_num=input_prepare_pack(os.path.join(main_path,folder),img_size,label_raw,brt_hue_round,crop_round,True,raw_size,new_size)
...
def input_prepare_pack(filepath,img_size,label_raw,brt_hue_round,crop_round,resize=True,raw_size=[512,512],new_size=[224,224]):  #for one class
...
def label_pre_pack(folder_name):
    ...
    label=[256,256,300,300,label_l[0]]
...
def change_brightness_hue_randomly(img,label,change_round,clip=None):
...
if __name__=='__main__':
    ...
    #img,label,num=input_prepare_pack('H:/DLtest/cool/1',[[512,512],512**2],brt_hue_round=1,resize=True,new_size=[224,224])
      get_all_done('H:/TRUEDATA/B/c/1/I','H:/DLc103',[[1080,1920],1080*1920],1024,brt_hue_round=0,crop_round=2,resize=True,raw_size=[512,512],new_size=[224,224])