...
def get_all_done(main_path,TF_savepath,img_size,file_size,brt_hue_round=1,crop_round=2,resize=True,new_size=[224,224]):
    ...
    for j,folder in enumerate(file_folders_class):
        ...
        if  img_num>file_size:           
            sidx=file_size
            TFRecord_writer(writer,img[0:sidx],label)
            writer.close()
            file_num+=1            
            img_num-=file_size
...
def input_prepare_pack(filepath,img_size,brt_hue_round=5,crop_round=6,resize=True,new_size=[224,224]):  #for one class
    ...
    c=img.shape[-1]
    if brt_hue_round>0:
        img = change_brightness_hue_randomly(img,b,c,brt_hue_round)
    img = change_contrast(img,b,c)
    img = noisy_img_randomly(img,[b,img_size[0][0],img_size[0][1],c])
    img = flip_it_randomly(img)  

    img =return_to_or_shape(img,img_size[0],c)
    if crop_round>0:
        img = crop_randomly(img,img_size[0],crop_round)
    if resize:
...
def change_brightness_hue_randomly(img,b=None,c=None,change_round=5,clip=None):
    if b==None:
        b=img.shape[0]
    if c==None:
        c=img.shape[-1]
...
def change_contrast(img,b=None,c=None,val=None):
    if b==None:
        b=img.shape[0]
    if c==None:
        c=img.shape[-1]
...
if __name__=='__main__':
    ...
    with tf.Session() as sess:
        get_all_done('H:/DLtest/A','H:/DLtest/TF',[[512,512],512**2],128,2,1)
